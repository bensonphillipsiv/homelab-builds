...
from .config import HOMEASSISTANT_URL, HOMEASSISTANT_TOKEN, get_ha_headers
...
def handle_api_errors(func: F) -> F:
    ...
    async def wrapper(*args: Any, **kwargs: Any) -> Any:
        ...
        try:
            ...
            return format_error(f"Connection error: Cannot connect to Home Assistant at {HOMEASSISTANT_URL}")
        except httpx.TimeoutException:
            return format_error(f"Timeout error: Home Assistant at {HOMEASSISTANT_URL} did not respond in time")
...
async def get_all_entity_states() -> Dict[str, Dict[str, Any]]:
    ...
    response = await client.get(f"{HOMEASSISTANT_URL}/api/states", headers=get_ha_headers())
...
async def get_hass_version() -> str:
    ...
    client = await get_client()
    response = await client.get(f"{HOMEASSISTANT_URL}/api/config", headers=get_ha_headers())
    response.raise_for_status()
    data = response.json()
...
async def get_entity_state(
    ...
    response = await client.get(
        f"{HOMEASSISTANT_URL}/api/states/{entity_id}", 
...
async def call_service(domain: str, service: str, data: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    response = await client.post(
        f"{HOMEASSISTANT_URL}/api/services/{domain}/{service}", 
        headers=get_ha_headers(),
        json=data
    )
    response.raise_for_status()
...
async def get_hass_error_log() -> Dict[str, Any]:
    ...
    try:
        ...
        url = f"{HOMEASSISTANT_URL}/api/error_log"
        headers = get_ha_headers()
...
async def get_entity_history(entity_id: str, hours: int) -> List[Dict[str, Any]]:
    ...
    url = f"{HOMEASSISTANT_URL}/api/history/period/{start_time_iso}"
